...
public class Parameter {
    ...
    public enum ParseStatus {MISSING, WRONG_TYPE, SUCCESS};
    public enum ValueType {STRING, DOUBLE, POSITIVE_DOUBLE, NONNEGATIVE_DOUBLE,
	    LONG, NONNEGATIVE_LONG, INT, NONNEGATIVE_INT, POSITIVE_INT};
    private ParseStatus parseStatus;
    private ValueType readyType;
    private int parseIndex;
    private String stringValue;
    private double doubleValue;
    private double positiveDoubleValue;
    private double nonnegativeDoubleValue;
    private long longValue;
    private long nonnegativeLongValue;
    private int intValue;
    private int nonnegativeIntValue;
    ...
    /*
     * New protocol: Parsing is now separated from value retrieval,
     * e.g.  1) client invokes parseDouble to parse an item as a
     * double; 2) client checks parseStatus to check for successful
     * parsing; 3) on good status, client calls getParsedDouble to get
     * actual value.  This allows the client to decide how to respond
	...
	if (!found) {
	    die("[Parameter.checkParseStatus] " + label
		+ " found flag is false");
	}
	switch (parseStatus) {
	case SUCCESS:
	...
	case MISSING:
	    die("[Parameter.checkParseStatus] " + label
		+ " parse failed: missing value at index " + parseIndex);
	    break;
	case WRONG_TYPE:
	    die("[Parameter.checkParseStatus] " + label
		+ " parse failed: wrong type:  " + values[parseIndex]
		+ " at index " + parseIndex);
	    break;
	default:
	    die("{Parameter.checkParse] unexpected status: "
		+ parseStatus);
	}
    }

    public ParseStatus parseString(int index) {
	if (!found) {
	    if (fileName != null) {
		die("Parameter: " + label + " not found in file " + fileName);
	    }
	    else {
		die("[Parameter.parseString] parmeter " + label
		    + "found flag is false" );
	    }
	}
	parseIndex = index;
	if (values == null) {
	    die("[Parameter.parseString] values array is null");
	}
	if (index < 0 || index >= values.length) {
	    die("[Parameter.parseString] index " + index + " does not exist!");
	}
	...
	checkParseStatus();
	if (readyType != ValueType.STRING) {
	    die("[Parameter.getParsedString] Parsed " + label 
		+ " value is not a string: "
		+ values[parseIndex] + " at index " + parseIndex);
	}
	return stringValue;
    }

    public ParseStatus  parseDouble(int index) {
	parseString(index);
	if (parseStatus == ParseStatus.SUCCESS) {
	    try {
		String val = getParsedString();
		doubleValue = Double.parseDouble(val);
		if (doubleValue == Double.NEGATIVE_INFINITY
		    || doubleValue == Double.POSITIVE_INFINITY) {
		    parseStatus = ParseStatus.WRONG_TYPE;
		}
		readyType = ValueType.DOUBLE;
	    }
	    catch (Exception e) {
		parseStatus = ParseStatus.WRONG_TYPE;
	    }
	...
	checkParseStatus();
	if (readyType != ValueType.DOUBLE) {
	    die("[Parameter.getParsedDouble] Parsed " + label 
    ...
    public ParseStatus parsePositiveDouble(int index) {
	parseDouble(index);
	if (parseStatus == ParseStatus.SUCCESS) {
	    if (doubleValue > 0) {
		positiveDoubleValue = doubleValue;
		readyType = ValueType.POSITIVE_DOUBLE;
	    }
	    else {
		parseStatus = ParseStatus.WRONG_TYPE;
	    }
    ...
    public double getParsedPositiveDouble() {
	checkParseStatus();
	if (readyType != ValueType.POSITIVE_DOUBLE) {
	    die("[Parameter.getParsedPositiveDouble] Parsed " + label 
		+ " value is not a positive double: "
		+ values[parseIndex] + " at index " + parseIndex);
	}
	return positiveDoubleValue;
    }

    public ParseStatus parseNonnegativeDouble(int index) {
	parseDouble(index);
	if (parseStatus == ParseStatus.SUCCESS) {
	    if (doubleValue >= 0) {
		nonnegativeDoubleValue = doubleValue;
		readyType = ValueType.NONNEGATIVE_DOUBLE;
	    }
	    else {
		parseStatus = ParseStatus.WRONG_TYPE;
	    }
    ...
    public double getParsedNonnegativeDouble() {
	checkParseStatus();
	if (readyType != ValueType.NONNEGATIVE_DOUBLE) {
	    die("[Parameter.getParsedNonnegativeDouble] Parsed " + label 
		+ " value is not a nonnegative double: "
		+ values[parseIndex] + " at index " + parseIndex);
	}
	return nonnegativeDoubleValue;
    }
	

    public ParseStatus parseLong(int index) {
	parseString(index);
	if (parseStatus == ParseStatus.SUCCESS) {
	    try {
		String val = getParsedString();
		longValue = Long.parseLong(val);
		readyType = ValueType.LONG;
	    }
	    catch (Exception e) {
		parseStatus = ParseStatus.WRONG_TYPE;
	    }
	...
	checkParseStatus();
	if (readyType != ValueType.LONG) {
	    die("[Parameter.getParsedLong] Parsed " + label 
		+ " value is not a long: "
		+ values[parseIndex] + " at index " + parseIndex);
	}
	return longValue;
    ...
    public ParseStatus parseNonnegativeLong(int index) {
	parseLong(index);
	if (parseStatus == ParseStatus.SUCCESS) {
	    if (longValue >= 0) {
		nonnegativeLongValue = longValue;
		readyType = ValueType.NONNEGATIVE_LONG;
	    }
	    else {
		parseStatus = ParseStatus.WRONG_TYPE;
	    }
    ...
    public long getParsedNonnegativeLong() {
	checkParseStatus();
	if (readyType != ValueType.NONNEGATIVE_LONG) {
	    die("[Parameter.getParsedNonnegativeLong] Parsed " + label 
		+ " value is not a nonnegative long: "
		+ values[parseIndex] + " at index " + parseIndex);
	}
	return nonnegativeLongValue;
    }


    public ParseStatus parseInt(int index) {
	parseString(index);
	if (parseStatus == ParseStatus.SUCCESS) {
	    try {
		String val = getParsedString();
		...
		if (lng < Integer.MIN_VALUE
		    || lng > Integer.MAX_VALUE) {
		    parseStatus = ParseStatus.WRONG_TYPE;
		}
		readyType = ValueType.INT;
	    }
	    catch (Exception e) {
		parseStatus = ParseStatus.WRONG_TYPE;
	    }
	...
	checkParseStatus();
	if (readyType != ValueType.INT) {
	    die("[Parameter.getParsedInt] Parsed " + label 
		+ " value is not an int: "
		+ values[parseIndex] + " at index " + parseIndex);
	}
	return intValue;
    ...
    public ParseStatus parseNonnegativeInt(int index) {
	parseInt(index);
	if (parseStatus == ParseStatus.SUCCESS) {
	    if (intValue >= 0) {
		nonnegativeIntValue = intValue;
		readyType = ValueType.NONNEGATIVE_INT;
	    }
	    else {
		parseStatus = ParseStatus.WRONG_TYPE;
	    }
    ...
    public int getParsedNonnegativeInt() {
	checkParseStatus();
	if (readyType != ValueType.NONNEGATIVE_INT) {
	    die("[Parameter.getParsedNonnegativeInt] Parsed " + label 
		+ " value is not a nonnegative int: "
		+ values[parseIndex] + " at index " + parseIndex);
	}
	return nonnegativeIntValue;
    }

    public ParseStatus parsePositiveInt(int index) {
	parseInt(index);
	if (parseStatus == ParseStatus.SUCCESS) {
	    if (intValue > 0) {
		positiveIntValue = intValue;
		readyType = ValueType.POSITIVE_INT;
	    }
	    else {
		parseStatus = ParseStatus.WRONG_TYPE;
	    }
    ...
    public int getParsedPositiveInt() {
	checkParseStatus();
	if (readyType != ValueType.POSITIVE_INT) {
	    die("[Parameter.getParsedPositiveInt] Parsed " + label 
		+ " value is not a positive int: "
		+ values[parseIndex] + " at index " + parseIndex);
	}
    ...
    public String getStringNEW(int index) {
	parseString(index);
	if (parseStatus != parseStatus.SUCCESS) {
	    die("[Parameter.getString]");
	}
	...
	if (index < 0 || index >= values.length) {
	    die("[Parameter.getString] index " + index + " does not exist!");
	}
	String val = values[index];
	if (val == null) {
	    die("Missing " + label + " parameter(s) on line " + lineNumber + " of file "
	...
	case SUCCESS:
	    break;
	case MISSING:
	    die("Missing " + label + " parameter(s) on line " + lineNumber
		+ " of file " + fileName);
	    break;
	case WRONG_TYPE:
	    die("[Parameter.getString] Could not interpret " + values[index]
		+ " as a string");
	    break;
	default:
	    die("[Parameter.getString] Unexpected parse status: " + parseStatus);
	}
	return stringValue;
    }
    ...
    //    public double getDoubleNEW(int index) {
    //	if (!found) {
    //	    die("Parameter: " + label + " not found in file " + fileName);
    //	}
    //	parseDouble(index);
